...
public final class UIFactory {
     ...
     */
    // Публічний, бо екран завантаження генерує ВЛАСНИЙ шрифт (він мусить
    // пережити disposeAll() посеред підготовки матчу) і мусить узяти той самий
    // набір: інакше кирилиця там просто не намалюється.
    ...
    /**
     * Пластина рядка, обведена кольором ГРАВЦЯ.
     *
     * <p>Заливка звичайна, а рамка — колір власника. Це єдиний спосіб дати
     * квадратику колір, не заливаючи його: залитий кольором гравця квадрат
     * сперечався б і з акцентом, і з кольорами сторін, а рамка в один піксель
     * читається як позначка, а не як тло (DESIGN §1).
     */
    public static Drawable createOwnerBackground(Color owner) {
        return rectPatch(ROW_BG, owner);
    }

    // ── Кольори гравців ───────────────────────────────────────────────────

    /**
     * Палітра власників — по одному кольору на гравця в команді.
     *
     * <p>Це НЕ кольори сторін: сторін дві, а гравців до п'яти на кожній, і
     * розрізняти союзників між собою синім і червоним неможливо. Тон тут
     * потрібен рівно для одного питання: «чиє це військо». Тому кольори взяті
     * помітно різні за відтінком, а не за яскравістю — на карті вони лягають
     * тонкою обводкою поверх строкатої місцевості.
     *
     * <p>Порядок фіксований: {@code colorIndex} приїжджає з лоббі й мусить дати
     * той самий тон у всіх, хто дивиться.
     */
    private static final Color[] OWNER_COLORS = {
        new Color(0.40f, 0.72f, 1.00f, 1f),   // блакитний
        new Color(1.00f, 0.78f, 0.28f, 1f),   // бурштин
        new Color(0.55f, 0.90f, 0.45f, 1f),   // зелень
        new Color(0.85f, 0.55f, 1.00f, 1f),   // бузок
        new Color(1.00f, 0.55f, 0.35f, 1f),   // теракота
        new Color(0.45f, 0.95f, 0.90f, 1f),   // бірюза
        new Color(0.95f, 0.45f, 0.65f, 1f),   // маджента
        new Color(0.75f, 0.75f, 0.55f, 1f),   // хакі
        new Color(0.60f, 0.65f, 1.00f, 1f),   // барвінок
        new Color(0.90f, 0.90f, 0.90f, 1f)    // біле
    };

    /** Колір власника за індексом із лоббі. Індекс поза межами загортається. */
    public static Color playerColor(int colorIndex) {
        int i = colorIndex % OWNER_COLORS.length;
        if (i < 0) i += OWNER_COLORS.length;
        return new Color(OWNER_COLORS[i]);
    }
